...
extern "C" {
bool chol_FactorizeAndSolveFwd_a(double* triang, unsigned N, double* result);
bool chol_Factorize_a(double* triang, unsigned N);
void chol_SolveFwd_a(double *x, unsigned N, const double* triang);
void chol_SolveBwd_a(double *x, unsigned N, const double* triang);
}
...
inline bool chol_FactorizeAndSolveFwd(double* triang, unsigned N, double* result, const bool solveFwd)
{
  ...
  if ((N & 1) != 0) { // special handling for the first row of matrix with odd number of elements
    ...
    do {
      ...
      do {
        ...
        __m256d y0_re = _mm256_loadu_pd(&y0[idx+0]);
        __m256d y0_im = _mm256_loadu_pd(&y0[idx+4]);
        __m256d y1_re = _mm256_loadu_pd(&y1[idx+0]);
        __m256d y1_im = _mm256_loadu_pd(&y1[idx+4]);
        ...
        _mm256_storeu_pd(&y0[idx+0], y0_re);
        _mm256_storeu_pd(&y0[idx+4], y0_im);
        _mm256_storeu_pd(&y1[idx+0], y1_re);
        _mm256_storeu_pd(&y1[idx+4], y1_im);
        idx += 8;
      } while (--cnt);
      #endif
      xi += 2;
  ...
  for (unsigned rhlen = N/2; ; rhlen-=1) {
    ...
    double aa1InvSqrt = aaInvSqrt[1]*aa0Sqrt;
    double aa1Sqrt    = aaSqrt[1]*aa0InvSqrt;
    x0[4*0+xi*2+0] = aa0Sqrt;
    x0[4*1+xi*2+0] = aa0InvSqrt;
    x1[4*0+xi*2+1] = aa1Sqrt;
    ...
    if (!solveFwd) {
      ...
      #endif
      for (int c = 0; c < cqlen; ++c) {
        __m256d x0_re = _mm256_mul_pd(_mm256_loadu_pd(&x0[c*8+0]), aa0InvSqrtV);
        __m256d x0_im = _mm256_mul_pd(_mm256_loadu_pd(&x0[c*8+4]), aa0InvSqrtV);
        __m256d x1_re = _mm256_loadu_pd(&x1[c*8+0]);
        __m256d x1_im = _mm256_loadu_pd(&x1[c*8+4]);
        __m256d r_re  = _mm256_loadu_pd(&pr[c*8+0]);
        __m256d r_im  = _mm256_loadu_pd(&pr[c*8+4]);

        _mm256_storeu_pd(&x0[c*8+0], x0_re);
        _mm256_storeu_pd(&x0[c*8+4], x0_im);
        ...
        _mm256_storeu_pd(&x1[c*8+0], x1_re);
        _mm256_storeu_pd(&x1[c*8+4], x1_im);
        ...
        _mm256_storeu_pd(&pr[c*8+0], r_re);
        _mm256_storeu_pd(&pr[c*8+4], r_im);
      }
      #endif
    ...
    for (unsigned chlen = rhlen-1; chlen > 0; chlen -= 1) {
      ...
      #else
      __m256d f00_re = _mm256_set1_pd(x0[4*0+xi*2+0]);
      __m256d f10_re = _mm256_set1_pd(x1[4*0+xi*2+0]);
      __m256d f01_re = _mm256_set1_pd(x0[4*0+xi*2+1]);
      __m256d f11_re = _mm256_set1_pd(x1[4*0+xi*2+1]);
      __m256d f00_im = _mm256_set1_pd(x0[4*1+xi*2+0]);
      __m256d f10_im = _mm256_set1_pd(x1[4*1+xi*2+0]);
      __m256d f01_im = _mm256_set1_pd(x0[4*1+xi*2+1]);
      __m256d f11_im = _mm256_set1_pd(x1[4*1+xi*2+1]);
      #ifdef __clang__
      #pragma unroll 1
      #endif
      for (int c = 0; c < cqlen; ++c) {
        // y0[c] = y0[c] - x0[c]*conj(f00) - x1[c]*conj(f10);
        // y1[c] = y1[c] - x0[c]*conj(f01) - x1[c]*conj(f11);
        __m256d x0_re = _mm256_loadu_pd(&x0[c*8+0]);
        __m256d x0_im = _mm256_loadu_pd(&x0[c*8+4]);
        __m256d y0_re = _mm256_loadu_pd(&y0[c*8+0]);
        __m256d y0_im = _mm256_loadu_pd(&y0[c*8+4]);
        __m256d y1_re = _mm256_loadu_pd(&y1[c*8+0]);
        __m256d y1_im = _mm256_loadu_pd(&y1[c*8+4]);
        ...
        _mm256_storeu_pd(&y0[c*8+0], y0_re);
        _mm256_storeu_pd(&y0[c*8+4], y0_im);
        _mm256_storeu_pd(&y1[c*8+0], y1_re);
        _mm256_storeu_pd(&y1[c*8+4], y1_im);
      }
      #endif
      y += cqlen*4*4;
      x0 += xi * 8;
...
{
  // double* dummy = new double[N*2+8];
  // memset(dummy, 0, sizeof(*dummy)*(N*2+8));
  // bool succ =  chol_FactorizeAndSolveFwd_a(triang, N, dummy);
  // delete [] dummy;
  // for (int i = 0; i < 16; ++i)
    // printf("%2d %f\n", i, triang[i]);
  // return succ;
...
#else
  return chol_Factorize_a(triang, N);
#endif
}
...
#else
  return chol_FactorizeAndSolveFwd_a(triang, N, result);
#endif
}
...
#if 0
void chol_SolveBwd_ref(double *x, unsigned N, const double* triang)
{
  // x = conj(R) \ x;
...
{
#if 1
  chol_SolveBwd_a(x, N, triang);
#else
  const int NX = (N+3) & -4;
  double* xref = new double[NX*2];
  memcpy(xref, x, sizeof(double)*NX*2);
  chol_SolveBwd_ref(xref, N, triang);
  chol_SolveBwd_a(x, N, triang);
  for (int i = 0; i < NX*2; ++i)
    printf("%2d %20.8f %20.8f\n", i, xref[i], x[i]);
  printf("\n");
  delete [] xref;
#endif
}
