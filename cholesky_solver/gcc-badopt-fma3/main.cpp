...
#include "chol.h"
extern unsigned long long gl_ticks;
...
int main(int argz, char** argv)
{
  if (argz < 2)
  {
    fprintf(stderr,
      ...
      " -r  - row-wise layout of input matrix, as standard C/C++.\n"
      " -c  - column-wise layout of input matrix, like in Fortran/Matlab\n"
      " -p  - packed column-wise layout of input matrix, lower triangle only. It is a default.\n"
  ...
  unsigned long long dticks[N_REPS];
  ...
  std::nth_element(&dticks[0], &dticks[N_REPS/2], &dticks[N_REPS]); // median
  ...
  if (profileTrifSolver)
    flops += substFlops;
  printf("N=%4d. %lld ticks. %6.2f usec. %.3f GMADD/s\n", n, dticks[N_REPS/2]/N_TEST_BUFFERS, tmPerTest*1E6, flops/(tmPerTest*1E9));
