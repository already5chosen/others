...
int main(int argz, char** argv)
{
  ...
  unsigned long long dticks1[N_REPS];
  unsigned long long dticks2[N_REPS];
  for (int rep = 0; rep < N_REPS; ++rep)
  {
    ...
    dt[rep] = t1 - t0;
    dticks1[rep] = gl_ticks[0];
    dticks2[rep] = ti1 - ti0;
  }

  std::nth_element(&dt[0], &dt[N_REPS/2], &dt[N_REPS]); // median
  std::nth_element(&dticks1[0], &dticks1[N_REPS/2], &dticks1[N_REPS]); // median
  std::nth_element(&dticks2[0], &dticks2[N_REPS/2], &dticks2[N_REPS]); // median
  ...
  if (profileTrifSolver)
    flops += substFlops;
  printf("N=%4d. %5.2f usec. uut %lld. tot %lld %.3f GMADD/s\n"
    , n
    , tmPerTest*1E6
    , dticks1[N_REPS/2]/N_TEST_BUFFERS
    , dticks2[N_REPS/2]/N_TEST_BUFFERS
    , flops/(tmPerTest*1E9)
    );
