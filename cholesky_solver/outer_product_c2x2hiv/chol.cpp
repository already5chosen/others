...
inline bool chol_FactorizeAndSolveFwd(double* triang, unsigned N, double* result, const bool solveFwd)
{
  ...
  if ((N & 1) != 0) { // special handling for the first row of matrix with odd number of elements
    ...
    do {
      ...
      #else
      __m256d f00_re = _mm256_set1_pd(x0[4*0+(xi&3)+0]);
      __m256d f00_im = _mm256_set1_pd(x0[4*1+(xi&3)+0]);
      __m256d f01_re = _mm256_set1_pd(x0[4*0+(xi&3)+1]);
      __m256d f01_im = _mm256_set1_pd(x0[4*1+(xi&3)+1]);
      int cnt = ylen2, idx = 0;
      #ifdef __clang__
      #pragma unroll 1
      #endif
      do {
        // y0[c] -= x0[c]*conj(f00);
        // y1[c] -= x0[c]*conj(f01);
        __m256d x_re  = _mm256_loadu_pd(&x0[idx+0]);
        __m256d x_im  = _mm256_loadu_pd(&x0[idx+4]);
        __m256d y0_re = _mm256_loadu_pd(&y[0+idx*2+0]);
        __m256d y0_im = _mm256_loadu_pd(&y[0+idx*2+4]);
        __m256d y1_re = _mm256_loadu_pd(&y[8+idx*2+0]);
        __m256d y1_im = _mm256_loadu_pd(&y[8+idx*2+4]);

        MSUB(y0_re, x_re, f00_re);
        MADD(y0_im, x_re, f00_im);
        MSUB(y1_re, x_re, f01_re);
        MADD(y1_im, x_re, f01_im);

        MSUB(y0_re, x_im, f00_im);
        MSUB(y0_im, x_im, f00_re);
        MSUB(y1_re, x_im, f01_im);
        MSUB(y1_im, x_im, f01_re);

        _mm256_storeu_pd(&y[0+idx*2+0], y0_re);
        _mm256_storeu_pd(&y[0+idx*2+4], y0_im);
        _mm256_storeu_pd(&y[8+idx*2+0], y1_re);
        _mm256_storeu_pd(&y[8+idx*2+4], y1_im);
        idx += 8;
      } while (cnt -= 8);
      #endif
      xi += 2;
  ...
  for (int rlen = N & -2; ; rlen -=2) {
    ...
    if (solveFwd) {
      ...
      r0_re =  result[0+xi+0] * aa0InvSqrt;
      r0_im =  result[4+xi+0] * aa0InvSqrt;
      r1_re = (result[0+xi+1] - f_re*r0_re + f_im*r0_im) * aa1InvSqrt;
      r1_im = (result[4+xi+1] - f_re*r0_im - f_im*r0_re) * aa1InvSqrt;
    }

    if (rlen <= 2) {
      triang[0+0+xi+0] = aa0Sqrt;
    ...
    if (!solveFwd) {
      ...
      for (int c = 0; c < clen*2; c += 8) {
        // auto ax0 = x0[c] * aa0InvSqrt;
        // auto ax1 = (x1[c] - ax0*conj(f))*aa1InvSqrt;;
        // x0[c] = ax0;
        // x1[c] = ax1
        // result[c] -= ax0[c]*r0 + ax1*r1;
        for (int k = 0; k < 4; ++k) {
          auto x0_re = x[0+c*2+0+k] * aa0InvSqrt;
          auto x0_im = x[0+c*2+4+k] * aa0InvSqrt;
          auto x1_re = x[8+c*2+0+k];
          auto x1_im = x[8+c*2+4+k];
          x[0+c*2+0+k] = x0_re;
          x[0+c*2+4+k] = x0_im;
          x1_re = (x1_re - x0_re*f_re - x0_im*f_im) * aa1InvSqrt;
          x1_im = (x1_im + x0_re*f_im - x0_im*f_re) * aa1InvSqrt;
          x[8+c*2+0+k] = x1_re;
          x[8+c*2+4+k] = x1_im;
          auto r_re  = pr[c+0+k];
          auto r_im  = pr[c+4+k];
      ...
      result[4+xi+1] = r1_im;
      #else
      __m256d aa0InvSqrtV = _mm256_set1_pd(aa0InvSqrt);
      __m256d aa1InvSqrtV = _mm256_set1_pd(aa1InvSqrt);
      __m256d f_reV = _mm256_set1_pd(f_re);
      __m256d f_imV = _mm256_set1_pd(f_im);
      __m256d r0_reV = _mm256_set1_pd(r0_re);
      __m256d r0_imV = _mm256_set1_pd(r0_im);
      __m256d r1_reV = _mm256_set1_pd(r1_re);
      __m256d r1_imV = _mm256_set1_pd(r1_im);
      #ifdef __clang__
      #pragma unroll 1
      #endif
      for (int c = 0; c < clen*2; c += 8) {
        __m256d x0_re = _mm256_mul_pd(_mm256_loadu_pd(&x[0+c*2+0]), aa0InvSqrtV);
        __m256d x0_im = _mm256_mul_pd(_mm256_loadu_pd(&x[0+c*2+4]), aa0InvSqrtV);
        __m256d x1_re = _mm256_loadu_pd(&x[8+c*2+0]);
        __m256d x1_im = _mm256_loadu_pd(&x[8+c*2+4]);
        __m256d r_re  = _mm256_loadu_pd(&pr[c+0]);
        __m256d r_im  = _mm256_loadu_pd(&pr[c+4]);

        _mm256_storeu_pd(&x[0+c*2+0], x0_re);
        _mm256_storeu_pd(&x[0+c*2+4], x0_im);

        MSUB(x1_re, x0_re, f_reV);
        MADD(x1_im, x0_re, f_imV);
        MSUB(r_re,  x0_re, r0_reV);
        MSUB(r_im,  x0_re, r0_imV);

        MSUB(x1_re, x0_im, f_imV);
        MSUB(x1_im, x0_im, f_reV);
        MADD(r_re,  x0_im, r0_imV);
        MSUB(r_im,  x0_im, r0_reV);

        x1_re = _mm256_mul_pd(x1_re, aa1InvSqrtV);
        x1_im = _mm256_mul_pd(x1_im, aa1InvSqrtV);

        _mm256_storeu_pd(&x[8+c*2+0], x1_re);
        _mm256_storeu_pd(&x[8+c*2+4], x1_im);

        MSUB(r_re,  x1_re, r1_reV);
        MSUB(r_im,  x1_re, r1_imV);
        MADD(r_re,  x1_im, r1_imV);
        MSUB(r_im,  x1_im, r1_reV);

        _mm256_storeu_pd(&pr[c+0], r_re);
        _mm256_storeu_pd(&pr[c+4], r_im);
      }
      triang[0+0+xi+0] = aa0Sqrt;
      triang[0+4+xi+0] = aa0InvSqrtV[0];
      triang[0+0+xi+1] = f_reV[0];
      triang[0+4+xi+1] = f_imV[0];
      triang[8+0+xi+1] = aa1Sqrt;
      triang[8+4+xi+1] = aa1InvSqrtV[0];
      result[0+xi+0] = r0_reV[0];
      result[0+xi+1] = r1_reV[0];
      result[4+xi+0] = r0_imV[0];
      result[4+xi+1] = r1_imV[0];
      #endif
      triang += (rlen + xi)*4;
      result = pr;
    }
    ...
    for (int clen = rlen-2; clen > 0; clen -= 2) {
      ...
      #else
      __m256d f00_re = _mm256_set1_pd(x[0+0+xi+0]);
      __m256d f10_re = _mm256_set1_pd(x[8+0+xi+0]);
      __m256d f01_re = _mm256_set1_pd(x[0+0+xi+1]);
      __m256d f11_re = _mm256_set1_pd(x[8+0+xi+1]);
      __m256d f00_im = _mm256_set1_pd(x[0+4+xi+0]);
      __m256d f10_im = _mm256_set1_pd(x[8+4+xi+0]);
      __m256d f01_im = _mm256_set1_pd(x[0+4+xi+1]);
      __m256d f11_im = _mm256_set1_pd(x[8+4+xi+1]);
      #ifdef __clang__
      #pragma unroll 1
      #endif
      for (int c = 0; c < ylen4; c += 16) {
        // y0[c] = y0[c] - x0[c]*conj(f00) - x1[c]*conj(f10);
        // y1[c] = y1[c] - x0[c]*conj(f01) - x1[c]*conj(f11);
        __m256d x0_re = _mm256_loadu_pd(&x[0+c+0]);
        __m256d x0_im = _mm256_loadu_pd(&x[0+c+4]);
        __m256d y0_re = _mm256_loadu_pd(&y[0+c+0]);
        __m256d y0_im = _mm256_loadu_pd(&y[0+c+4]);
        __m256d y1_re = _mm256_loadu_pd(&y[8+c+0]);
        __m256d y1_im = _mm256_loadu_pd(&y[8+c+4]);
        MSUB(y0_re, x0_re, f00_re);
        MSUB(y1_re, x0_re, f01_re);
        MADD(y0_im, x0_re, f00_im);
        MADD(y1_im, x0_re, f01_im);
        MSUB(y0_re, x0_im, f00_im);
        MSUB(y1_re, x0_im, f01_im);
        MSUB(y0_im, x0_im, f00_re);
        MSUB(y1_im, x0_im, f01_re);
        __m256d x1_re = _mm256_loadu_pd(&x[8+c+0]);
        __m256d x1_im = _mm256_loadu_pd(&x[8+c+4]);
        MSUB(y0_re, x1_re, f10_re);
        MSUB(y1_re, x1_re, f11_re);
        MADD(y0_im, x1_re, f10_im);
        MADD(y1_im, x1_re, f11_im);
        MSUB(y0_re, x1_im, f10_im);
        MSUB(y1_re, x1_im, f11_im);
        MSUB(y0_im, x1_im, f10_re);
        MSUB(y1_im, x1_im, f11_re);
        _mm256_storeu_pd(&y[0+c+0], y0_re);
        _mm256_storeu_pd(&y[0+c+4], y0_im);
        _mm256_storeu_pd(&y[8+c+0], y1_re);
        _mm256_storeu_pd(&y[8+c+4], y1_im);
      }
      #endif
      y += ylen4;
      x += xi * 8;
...
void chol_SolveFwd(double *x, unsigned N, const double* triang)
{
  ...
  if ((N & 1) != 0) {    // special handling for the first row of matrix with odd number of elements
    ...
    auto xr_re = x[xi+1+0] * aaInvSqrt;
    auto xr_im = x[xi+1+4] * aaInvSqrt;
    int clen = (N+1) & -4;
    auto x0 = x;
    if (clen != 0) {
      x += xi*4;
      ...
      for (int c = 0; c < clen*2; c += 8) {
        ...
        for (int k = 0; k < 4; ++k) {
          ...
          auto y_re = y[c+k+0];
          auto y_im = y[c+k+4];
          x[c+k+0] = x_re - y_re*xr_re + y_im*xr_im;
          x[c+k+4] = x_im - y_re*xr_im - y_im*xr_re;
        }
      }
      #else
      __m256d xr_reV = _mm256_set1_pd(xr_re);
      __m256d xr_imV = _mm256_set1_pd(xr_im);
      int cnt = clen*2, idx = 0;
      #ifdef __clang__
      #pragma unroll 1
      #endif
      do {
        __m256d x_re = _mm256_loadu_pd(&x[idx+0]);
        __m256d x_im = _mm256_loadu_pd(&x[idx+4]);
        __m256d y_re = _mm256_loadu_pd(&y[idx+0]);
        __m256d y_im = _mm256_loadu_pd(&y[idx+4]);
        MSUB(x_re, y_re, xr_reV);
        MSUB(x_im, y_re, xr_imV);
        MADD(x_re, y_im, xr_imV);
        MSUB(x_im, y_im, xr_reV);
        _mm256_storeu_pd(&x[idx+0], x_re);
        _mm256_storeu_pd(&x[idx+4], x_im);
        idx += 8;
      } while (cnt -= 8);
      xr_re = xr_reV[0];
      xr_im = xr_imV[0];
    ...
    if (clen != 0) {
      ...
      for (int c = 0; c < clen*2; c += 8) {
        ...
        for (int k = 0; k < 4; ++k) {
          auto x_re = x[c+k+0];
          auto x_im = x[c+k+4];
          auto y0_re = y[0+c*2+k+0];
          auto y0_im = y[0+c*2+k+4];
          auto y1_re = y[8+c*2+k+0];
          auto y1_im = y[8+c*2+k+4];
          x[c+k+0] = x_re - y0_re*xr0_re + y0_im*xr0_im - y1_re*xr1_re + y1_im*xr1_im;
          x[c+k+4] = x_im - y0_re*xr0_im - y0_im*xr0_re - y1_re*xr1_im - y1_im*xr1_re;
        }
      }
      #else
      __m256d xr0_reV = _mm256_set1_pd(xr0_re);
      __m256d xr0_imV = _mm256_set1_pd(xr0_im);
      __m256d xr1_reV = _mm256_set1_pd(xr1_re);
      __m256d xr1_imV = _mm256_set1_pd(xr1_im);
      int cnt = clen*2, idx = 0;
      #ifdef __clang__
      #pragma unroll 1
      #endif
      do {
        ...
        MSUB(x_re, y0_re, xr0_reV);
        MSUB(x_im, y0_re, xr0_imV);

        MADD(x_re, y0_im, xr0_imV);
        MSUB(x_im, y0_im, xr0_reV);

        MSUB(x_re, y1_re, xr1_reV);
        MSUB(x_im, y1_re, xr1_imV);

        MADD(x_re, y1_im, xr1_imV);
        MSUB(x_im, y1_im, xr1_reV);
...
void chol_SolveBwd(double *x, unsigned N, const double* triang)
{
  ...
  for (int rlen = 2; rlen <= int(N); rlen += 2) {
    ...
    #else
    __m256d acc0_rere = _mm256_setr_pd(x0[0+0],0,0,0);
    __m256d acc1_rere = _mm256_setr_pd(x0[1+0],0,0,0);
    __m256d acc0_imre = _mm256_setr_pd(x0[0+4],0,0,0);
    __m256d acc1_imre = _mm256_setr_pd(x0[1+4],0,0,0);
    x0[0+0]=x0[1+0] = 0;
    x0[0+4]=x0[1+4] = 0;
    __m256d acc0_imim = _mm256_setzero_pd(), acc1_imim = _mm256_setzero_pd();
    __m256d acc0_reim = _mm256_setzero_pd(), acc1_reim = _mm256_setzero_pd();
    int clen = rlen & -4;
    if (clen != 0) {
      int cnt = clen, idx = 0;
      #ifdef __clang__
      ...
      do {
        ...
        __m256d y1_re = _mm256_loadu_pd(&y[8+idx*2+0]);
        __m256d y1_im = _mm256_loadu_pd(&y[8+idx*2+4]);
        MSUB(acc0_rere, x_re, y0_re);
        MSUB(acc1_rere, x_re, y1_re);
        MADD(acc0_reim, x_re, y0_im);
        MADD(acc1_reim, x_re, y1_im);
        MSUB(acc0_imre, x_im, y0_re);
        MSUB(acc1_imre, x_im, y1_re);
        MADD(acc0_imim, x_im, y0_im);
        MADD(acc1_imim, x_im, y1_im);
        idx += 8;
      } while (cnt -= 4);
    }
    __m256d acc1_reV4 = _mm256_sub_pd(acc1_rere, acc1_imim);
    __m256d acc1_imV4 = _mm256_add_pd(acc1_imre, acc1_reim);
    __m256d acc0_reV4 = _mm256_sub_pd(acc0_rere, acc0_imim);
    __m256d acc0_imV4 = _mm256_add_pd(acc0_imre, acc0_reim);
    __m256d acc1_reimV4 = _mm256_hadd_pd(acc1_reV4, acc1_imV4);
    __m256d acc0_reimV4 = _mm256_hadd_pd(acc0_reV4, acc0_imV4);
    __m128d acc1_reimV2 = _mm_add_pd(_mm256_castpd256_pd128(acc1_reimV4), _mm256_extractf128_pd(acc1_reimV4, 1));
    __m128d acc0_reimV2 = _mm_add_pd(_mm256_castpd256_pd128(acc0_reimV4), _mm256_extractf128_pd(acc0_reimV4, 1));
    double acc1_re = acc1_reimV2[0];
    double acc1_im = acc1_reimV2[1];
    double acc0_re = acc0_reimV2[0];
    double acc0_im = acc0_reimV2[1];
    #endif
    y = &triang[xi];
  ...
  if ((N & 1) != 0) { // special handling for the first row of matrix with odd number of elements
    ...
    int clen = (N+1) & -4;
    #if 0
    auto acc_rere = x0[1+0];
    auto acc_reim = x0[1+4];
    double acc_imre = 0;
    double acc_imim = 0;
    x0[0+0]=x0[1+0]=0;
    x0[0+4]=x0[1+4]=0;
    for (int c = 0; c < clen*2; c += 8) {
      // acc -= x[c] * conj(y[c]);
      for (int k = 0; k < 4; ++k) {
        acc_rere -= x[c+k+0]*y[c+k+0];
        acc_reim += x[c+k+0]*y[c+k+4];
        acc_imre += x[c+k+4]*y[c+k+0];
        acc_imim += x[c+k+4]*y[c+k+4];
      }
    }
    auto acc_re = acc_rere - acc_imim;
    auto acc_im = acc_reim - acc_imre;
    #else
    __m256d acc_rere = _mm256_setr_pd(x0[1+0],0,0,0);
    __m256d acc_reim = _mm256_setr_pd(x0[1+4],0,0,0);
    x0[0+0]=x0[1+0] = 0;
    x0[0+4]=x0[1+4] = 0;
    __m256d acc_imim = _mm256_setzero_pd();
    __m256d acc_imre = _mm256_setzero_pd();
    if (clen != 0) {
      int cnt = clen, idx = 0;
      #ifdef __clang__
      #pragma unroll 1
      ...
      do {
        ...
        __m256d y_re = _mm256_loadu_pd(&y[idx+0]);
        __m256d y_im = _mm256_loadu_pd(&y[idx+4]);
        MSUB(acc_rere, x_re, y_re);
        MADD(acc_reim, x_re, y_im);
        MADD(acc_imre, x_im, y_re);
        MADD(acc_imim, x_im, y_im);
        idx += 8;
      } while (cnt -= 4);
    }
    __m256d acc_reV4 = _mm256_sub_pd(acc_rere, acc_imim);
    __m256d acc_imV4 = _mm256_sub_pd(acc_reim, acc_imre);
    __m256d acc_reimV4 = _mm256_hadd_pd(acc_reV4, acc_imV4);
    __m128d acc_reimV2 = _mm_add_pd(_mm256_castpd256_pd128(acc_reimV4), _mm256_extractf128_pd(acc_reimV4, 1));
    double acc_re = acc_reimV2[0];
    double acc_im = acc_reimV2[1];
    #endif
    auto aaInvSqrt = triang[xi+1+4]; // imag() of diag element contains inverse of its real()
