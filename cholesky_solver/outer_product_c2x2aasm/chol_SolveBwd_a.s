...
.seh_proc chol_SolveBwd_a
  ...
  .rloop:
    ...
    .cloop:
      ...
      vmulpd   %ymm4, %ymm5, %ymm6  # ymm6 = x[c]*y0[c] { re*re im*im re*re im*im }
      vsubpd   %ymm6, %ymm0, %ymm0  # ymm0 = acc0_re -= x[c]*y0[c] { re*re im*im re*re im*im }
      vpermilpd $5,   %ymm4, %ymm6  # ymm5 = vxx = x[c] { im re im re }
      vmulpd   %ymm6, %ymm5, %ymm5  # ymm5 = x[c]*y0[c] { im*re re*im im*re re*im }
      vsubpd   %ymm5, %ymm1, %ymm1  # ymm1 = acc0_im -= x[c]*y0[c] { im*re re*im im*re re*im }

      vmovupd  (%rax, %rdx), %ymm5  # ymm5 = y1[c]      { re im re im }
      add      $32,   %rax          # eax  = y0 += 2
      vmulpd   %ymm5, %ymm4, %ymm4  # ymm4 = x[c]*y1[c] { re*re im*im re*re im*im }
      vsubpd   %ymm4, %ymm2, %ymm2  # ymm2 = acc1_re -= x[c]*y1[c] { re*re im*im re*re im*im }
      vmulpd   %ymm5, %ymm6, %ymm6  # ymm5 = x[c]*y1[c] { im*re re*im im*re re*im }
      vsubpd   %ymm6, %ymm3, %ymm3  # ymm3 = acc1_im -= x[c]*y1[c] { im*re re*im im*re re*im }
